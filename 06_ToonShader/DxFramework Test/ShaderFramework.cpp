...
// 쉐이더
LPD3DXEFFECT gpToonShader = nullptr;

// 라이트 컬러
D3DXVECTOR4 gSurfaceColor(1.f, 0.f, 1.f, 1.f);

// 라이트 위치
D3DXVECTOR4 gWorldLightPosition(500.f, 500.f, -500.f, 1.f);
...
{
	// RotationY
	static float RotaionY = 0.f;
	RotaionY += 0.01f;
	
	// World matrix
	D3DXMATRIX matWorld;
	D3DXMatrixRotationY(&matWorld, RotaionY);
	...
	// View Matrix
	D3DXMATRIX matView;
	D3DXVECTOR3 eyeVec = {0.f, 0.f, -200.f};
	D3DXVECTOR3 lookVec = {0.f, 0.f, 0.f};
	D3DXVECTOR3 upVec = {0.f, 1.f, 0.f};
	D3DXMatrixLookAtLH(&matView, &eyeVec, &lookVec, &upVec);
	
	// Projection Matrix
	D3DXMATRIX matProjection;
	D3DXMatrixPerspectiveFovLH(&matProjection, 3.14159216f/4.f, WIN_WIDTH/(float)WIN_HEIGHT, 1.f, 10000.f);

	// WorldViewProjection Matrix
	D3DXMATRIX matWorldViewMatrix;
	D3DXMATRIX matWorldViewProjectionMatrix;
	D3DXMatrixMultiply(&matWorldViewMatrix, &matWorld, &matView);
	D3DXMatrixMultiply(&matWorldViewProjectionMatrix, &matWorldViewMatrix, &matProjection);
	
	// shader setting
	gpToonShader->SetMatrix("gWorldViewProjectionMatrix", &matWorldViewProjectionMatrix);
	...
	// render
	UINT PassesNum = 0;
	gpToonShader->Begin(&PassesNum, 0);
