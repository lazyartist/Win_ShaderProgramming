...
// 모델
LPD3DXMESH gpModel = nullptr;

// 쉐이더
LPD3DXEFFECT gpShader = nullptr;
D3DXVECTOR4 gWorldLightPosition(500.f, 500.f, -500.f, 1.f);
D3DXVECTOR4 gSurfaceColor(1.f, 0.f, 0.f, 1.f);
...
{
    // RotaionX
    static float RotaionX = 0.f;
    static float RotaionY = 0.f;
    static float RotaionZ = 0.f;
    RotaionX += 0.01f;
    RotaionY += 0.02f;
    RotaionZ += 0.03f;
    ...
    // World Matrix
    D3DXMATRIX matWorld;
    D3DXMatrixRotationYawPitchRoll(&matWorld, RotaionZ, RotaionY, RotaionX);
    ...
    D3DXVECTOR3 atVec{0.f, 0.f, 0.f};
    D3DXVECTOR3 upVec{0.f, 1.f, 0.f};
    D3DXMatrixLookAtLH(&matView, &eyeVec, &atVec, &upVec);
...
bool InitD3D(HWND hWnd)
{
    ...
    // 모니터 주사율과 백버퍼를 스왑하는 빈도간의 관계. D3DPRESENT_INTERVAL_ONE == 모니터가 동기될 때마다 백버퍼를 스왑. 이 때 Tearing 현상 발생.
...
bool LoadAssets()
{
    ...
    // 쉐이더 로딩
    gpShader = LoadShader("ToonShader.fx");
    if (nullptr == gpShader)
    {
        return false;
    }
