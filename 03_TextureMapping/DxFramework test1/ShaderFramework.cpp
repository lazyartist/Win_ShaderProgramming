...
// 쉐이더
LPD3DXEFFECT gpTextureMappingShader = nullptr;
...
void Update()
{
    gRotaionY += 1.f/*Speed*/ * 3.14f / 180.f;
}
...
void RenderScene()
{
    ...
    D3DXMatrixRotationY(&matWorld, gRotaionY);
    ...
    D3DXMatrixPerspectiveFovLH(&matProjection, 3.14f/4.f, static_cast<float>(WIN_WIDTH)/static_cast<float>(WIN_HEIGHT), 1.f, 10000.f);
    ...
    UINT numPasses;
    gpTextureMappingShader->Begin(&numPasses, 0);
...
bool LoadAssets()
{
    ...
    if(nullptr == gpTextureMappingShader)
    {
        return false;
    }
