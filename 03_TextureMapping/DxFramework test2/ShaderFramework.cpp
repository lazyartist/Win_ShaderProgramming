...
// 쉐이더
LPD3DXEFFECT gpTextureMapping = nullptr;

// 텍스처
LPDIRECT3DTEXTURE9 gpEarthDM = nullptr;
...
void Update()
{
    gRotationY += 1.0f * 3.14 / 180.f;
    if(3.14 * 2.f < gRotationY)
    {
        gRotationY -= 3.14 * 2.f;
    }
}
...
void RenderScene()
{
    ...
    D3DXMatrixPerspectiveFovLH(&matProjection, 3.14f/4.f, static_cast<float>(WIN_WIDTH)/static_cast<float>(WIN_HEIGHT), 1.f, 1000.f);
    
    // shader
    gpTextureMapping->SetMatrix("gWorldMatrix", &matWorld);
    gpTextureMapping->SetMatrix("gViewMatrix", &matView);
    gpTextureMapping->SetMatrix("gProjectionMatrix", &matProjection);
    gpTextureMapping->SetTexture("DiffuseMap_Tex", gpEarthDM);

    // draw
    UINT numPasses;
    gpTextureMapping->Begin(&numPasses, 0);
    for (int i = 0; i < numPasses; ++i)
    {
        gpTextureMapping->BeginPass(i);
        gpSphere->DrawSubset(0);
        gpTextureMapping->EndPass();
    }
    gpTextureMapping->End();
}
...
bool LoadAssets()
{
    ...
    if (nullptr == gpEarthDM)
    {
        return false;
    }

    // 쉐이더 로딩
    gpTextureMapping = LoadShader("TextureMapping.fx");
    if (nullptr == gpTextureMapping)
    {
        return false;
    }
...
void Cleanup()
{
    ...
    // 쉐이더를 release 한다.
    gpTextureMapping->Release();
    gpTextureMapping = nullptr;
