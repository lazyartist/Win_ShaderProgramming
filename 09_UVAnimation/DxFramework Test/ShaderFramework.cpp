...
void RenderScene()
{
	...
	gpUVAnimationShader->SetVector("gWorldLightColor", &gWorldLightColor);
    ...
    // render
    UINT numPass;
    gpUVAnimationShader->Begin(&numPass, 0);
...
bool LoadAssets()
{
    ...
    if(nullptr == gpUVAnimationShader)
    {
        return false;
    }

	// 모델 로딩
    gpTorus = LoadModel("../../Resources/Torus.x");
    if (nullptr == gpTorus)
    {
        return false;
    }
